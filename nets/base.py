...
class BaseNet(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # loss = self.loss_func(y_pred.squeeze() * 81, (y * 81).long())
        loss = self.loss_func(y_pred.squeeze() * 81, y * 81)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_pred = self(x)
        print(y_pred.device.type, y.device.type)
        # loss = self.loss_func(y_pred.squeeze() * 81, (y * 81).long())
        loss = self.loss_func(y_pred.squeeze() * 81, y * 81)

        self.validation_outputs.append({"loss": loss, "pred": y_pred, "y": y})
    ...
    def on_validation_epoch_end(self):
        ...
        # loss = self.loss_func(y_pred.squeeze() * 81, (y_true * 81).long())
        loss = self.loss_func(y_pred.squeeze() * 81, y_true * 81)
